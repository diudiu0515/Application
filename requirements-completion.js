(function(){
  ...
  function mainlandStatus(p){return p&&p.mainlandApplicantStatus||"needs_2028_official_check"}
  function summerStatus(p){
    var rows=(state.summerResearch||[]).filter(function(x){return x.school===(p&&p.school)}),order={eligible:4,likely_eligible:3,needs_confirmation:2,ineligible:1};
    return rows.length?rows.sort(function(a,b){return (order[b.internationalStatus]||0)-(order[a.internationalStatus]||0)})[0].internationalStatus:"no_record";
  }
  ...
  function facultyRows(){
    ...
    return state.faculty.filter(function(f){
      ...
      return (!filters.topic||text.includes(filters.topic.toLowerCase()))&&
        ...
        (!filters.mainland||mainlandStatus(p)===filters.mainland)&&
        (!filters.summer||summerStatus(p)===filters.summer)&&
